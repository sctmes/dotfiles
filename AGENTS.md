...
- Renovate may update only the `upstream` and public `yazelix` flake inputs automatically. Yazelix Nova `main` is a downstream-only experiment for `ysun`: install `yazelix-no-mars` and use `yzx enter`; do not expose it to other users or upstream profiles. Yazelix PRs may automerge only after the maintenance gate passes, and automerge never rebuilds host `116`.
