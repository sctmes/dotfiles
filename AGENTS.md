...
- Host `116` is a multi-user SSH development server. Shared runtime needed by all SSH users belongs in system-level host modules under `hosts/116/`, not in one user's Home Manager profile or ad hoc shell startup workaround.
- A global npm registry default is acceptable for the mainland China network path, but do not add noisy or unsupported npm config such as `NPM_CONFIG_DISTURL`.
...
- Renovate may update only the `upstream` and public `yazelix` flake inputs automatically. Yazelix Nova `edge` is a downstream-only experiment for `ysun`: install `yazelix-no-rio` and use `yzx enter`; do not expose it to other users or upstream profiles. This leaf repository does not require the maintenance gate for merging, but the operator must have successful gate evidence for the exact change before manually activating it on host `116`; merging never rebuilds the host.
- The CI cache gate may stub `yazelix-next` only for a legacy comparison snapshot that still declares that private input. The public Yazelix head and all post-migration comparisons must evaluate the real package.
