...
The installed system declares its steady-state proxy and cache policy through `hosts/116/proxy.nix`.
...
- `zky` and `wangrongfeng` get a minimal headless TUI setup:
  ...
  - `yazi`
  - `ripgrep`
- `nix-daemon` receives declarative proxy settings from `hosts/116/proxy.nix`, so post-install `nix` / `nixos-rebuild` use the host-local Mihomo proxy on `127.0.0.1:7890`
...
The installed system already points Nix traffic at the host-local proxy declared in `hosts/116/proxy.nix`.
...
4. Restore model files to `/var/lib/ai-serving/models` if the assistant stack is enabled.
5. Check the public Label Studio path:
